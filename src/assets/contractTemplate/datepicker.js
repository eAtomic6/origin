(function () {
    var utils = {
        ...
        fadeOut: function(target) {
          ...
          var tapElement = document.querySelector('input[' + Dropdown.originOpt.PANELKEY + '="' + wrapKey.split(Dropdown.originOpt.PANELSTR)[1] + '"]')
    ...
    function Calendar(elem,elemArr) {
        // 参数
    ...
    Calendar.create = function(opt) {
        ...
        for(var i=0;i<elemArr.length;i++){
            elemArr[i].calendar = new Calendar(elemArr[i],elemArr);
        }
        console.log(elemArr)
    }
    Calendar.originOpt = {
    ...
    Calendar.prototype = {
        ...
        openPanel: function (target,arr) {
            for(var i=0;i<arr.length;i++){//多个控件同时存在时，收起其他控件，保证只展示一个
                if(utils.hasClass(arr[i], Calendar.originOpt.PICKERNAME)&&target!==arr[i]){
                    var key = utils.attr(arr[i], Calendar.originOpt.PANELKEY)
                    arr[i].elem_wrap = document.querySelector('.' + Calendar.originOpt.PANELSTR + key);
        ...
        initEvent: function () {
            var self = this;
            this.elem_wrap.onclick=function(e){
                e.stopPropagation();
                var target = e.target;
            ...
            window.addEventListener('click', function() {
                var random = self.bindElem.getAttribute('random')
                var inputVal = 0
                if(random&&self.bindElem.value){
                    inputVal = new Date(random)
                }else {
                    inputVal = new Date()
                    self.dateOpt._date=inputVal.getDate()
                }
                if(self.dateOpt._year!==(inputVal.getFullYear())){
                    self.dateOpt._year=inputVal.getFullYear()
                    self.dateOpt._month=inputVal.getMonth()
                    self.updateHtml('yearChange')
                }else if(self.dateOpt._month!==(inputVal.getMonth())){
                    self.dateOpt._month=inputVal.getMonth()
                    self.updateHtml('monthChange')
                }else {
                    self.updateHtml('reset')
                }
        ...
        updateHtml: function (type) {
            ...
            if (this.isSelected) {
                ...
                Calendar.Opt.callBack && Calendar.Opt.callBack(this.bindElem, {
                    ...
                    random: this.dateOpt.selectYear+"/"+(this.dateOpt.selectMonth + 1)+"/"+this.dateOpt.selectDate
                });
            }
